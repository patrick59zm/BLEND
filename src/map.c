...
static int mm_dust_minier(void *km, int n, mm128_t *a, int l_seq, const char *seq, int sdust_thres)
{
	...
	for (j = k = 0; j < n; ++j) { // squeeze out minimizers that significantly overlap with LCRs
		int32_t qpos = (uint32_t)a[j].y>>1, span = a[j].x&0x3fff;
...
static void collect_minimizers(void *km, const mm_mapopt_t *opt, const mm_idx_t *mi, int n_segs, const int *qlens, const char **seqs, mm128_v *mv, mm128_v *nmv)
{
	...
	for (i = n = 0; i < n_segs; ++i) {
		...
		//@IMPORTANT: commented here. I'll study later its effects to the blend values instead of
		//minimizers
...
//                printf("%d rev %d", q->q_span, (qlen - ((q->q_pos>>1) + 1 - q->q_span) - 1));
			}
			p->y |= (uint64_t)q->seg_id << MM_SEED_SEG_SHIFT;
			if (q->is_tandem) p->y |= MM_SEED_TANDEM;
			if (is_self) p->y |= MM_SEED_SELF;

			// printf("%s2 span:%d pos:%d rpos:%d\n", __func__, q->q_span, q->q_pos, rpos);
			// static char cpyC[65];
			// cpyC[0] = '\0';
								  ...
								  int *n_mini_pos, uint64_t **mini_pos)
{
	...
	for (i = 0, *n_a = 0; i < n_m; ++i) {
		...
		uint32_t k;
		for (k = 0; k < q->n; ++k) { //q->n is the number of seed matches for certain seed
			int32_t is_self, rpos = (uint32_t)r[k] >> 1; //r[k] is a.y in ref sketch
			mm128_t *p;
			if (skip_seed(opt->flag, r[k], q, qname, qlen, mi, &is_self)) continue;
			p = &a[(*n_a)++];
			if ((r[k]&1) == (q->q_pos&1)) { // forward strand
				//p->x ref and p->y is query
