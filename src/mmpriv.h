...
#define MM_SEED_IGNORE     (1ULL<<47)
#define MM_SEED_TANDEM     (1ULL<<48)
#define MM_SEED_SELF       (1ULL<<49)

#define MM_SEED_SEG_SHIFT  50
...
void mm_sketch_blend(void *km, const char *str, int len, int w, int blend_bits, int k, int n_neighbors, uint32_t rid, int is_hpc, mm128_v *p, mm128_v *np);
 ...
 */
void mm_sketch(void *km, const char *str, int len, int w, int blend_bits, int k, int k_shift, int n_neighbors, uint32_t rid, int is_hpc, int is_skewed, int is_strobs, mm128_v *p, mm128_v *np);
